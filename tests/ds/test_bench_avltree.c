...
void test_avltree(void)
{
	...
	start = clock();
	for (int32_t i = 0; i < SIZE; i++) {
		if (!F_SYMBOL(avl_wrap_insert)(&root, &node[i].node, cmp))
	...
	/* first and last */
	p = F_SYMBOL(avl_wrap_first)(&root);
	printf("first: k:%d %p\n", container_of(p, struct T, node)->key, p);

	p = F_SYMBOL(avl_wrap_last)(&root);
	printf("last: k:%d %p\n", container_of(p, struct T, node)->key, p);

	/* search */
	start = clock();
	for (int32_t i = 0; i < SIZE; i++) {
		if (!F_SYMBOL(avl_wrap_search)(&root, &node[i].node, cmp))
	...
	for (int32_t i = 0; i < A_SIZE; i++) {
		if (!F_SYMBOL(avl_wrap_erase2)(&root, &node[i].node, cmp))
			printf("not found: i:%d k:%d\n", i, node[i].key);
	}
	end = clock();
	time = (double)(end - start) / CLOCKS_PER_SEC;
