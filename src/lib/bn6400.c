...
{
	C_SYMBOL(memcpy)(t, s, sizeof(bn6400_t));
}
...
{
	bn6400_t t;
	C_SYMBOL(memcpy)(&t, a, sizeof(bn6400_t));
	C_SYMBOL(memcpy)(a, b, sizeof(bn6400_t));
	C_SYMBOL(memcpy)(b, &t, sizeof(bn6400_t));
}
...
{
	C_SYMBOL(memset)(t, 0, sizeof(bn6400_t));
	t->n[0] = n;
}

/* @func: bn6400_from_bytes
 * #desc:
...
{
	F_SYMBOL(bn6400_set1)(t, 0);
	C_SYMBOL(memcpy)(t->n, s, len);
}

/* @func: bn6400_to_bytes
 * #desc:
		...
		const bn6400_t *a, const bn6400_t *b)
{
	...
	F_SYMBOL(bn6400_set1)(&rr, 0);

	int32_t a_len = F_SYMBOL(bn6400_bits)(a);
	int32_t b_len = F_SYMBOL(bn6400_bits)(b);
	if (!a_len || !b_len)
		goto e;
...
{
	bn6400_t rquo, rrem;
	F_SYMBOL(bn6400_move)(&rrem, a);
	F_SYMBOL(bn6400_move)(&rquo, b);

	if (rem)
		F_SYMBOL(bn6400_move)(rem, &rrem);
	if (quo)
		F_SYMBOL(bn6400_set1)(quo, 0);

	if (!F_SYMBOL(bn6400_ucmp_1)(&rquo, 0)) {
		if (rem)
			F_SYMBOL(bn6400_set1)(rem, 0);
		return;
	}
	if (F_SYMBOL(bn6400_ucmp)(&rrem, &rquo) < 0)
		return;

	int32_t a_len = F_SYMBOL(bn6400_bits)(&rrem);
	int32_t b_len = F_SYMBOL(bn6400_bits)(&rquo);
	if (!a_len || !b_len)
		return;
...
{
	bn6400_t t;
	F_SYMBOL(bn6400_set1)(&t, b);

	F_SYMBOL(bn6400_udiv)(quo, rem, a, &t);
}
		...
		const bn6400_t *a, const bn6400_t *b)
{
	...
	if (sign == b->sign) {
		F_SYMBOL(bn6400_uadd)(r, a, b);
		r->sign = sign;
		return;
	}

	int32_t k = F_SYMBOL(bn6400_ucmp)(a, b);
	if (k > 0) {
		F_SYMBOL(bn6400_usub)(r, a, b);
		r->sign = sign;
	} else if (k < 0) {
		sign = b->sign;
		F_SYMBOL(bn6400_usub)(r, b, a);
		r->sign = sign;
	} else {
		F_SYMBOL(bn6400_set1)(r, 0);
	}
		...
		const bn6400_t *a, const bn6400_t *b)
{
	...
	F_SYMBOL(bn6400_umul)(r, a, b);

	r->sign = F_SYMBOL(bn6400_bits)(r) ? sign : 0;
}
		...
		const bn6400_t *a, const bn6400_t *b)
{
	...
	F_SYMBOL(bn6400_udiv)(quo, rem, a, b);

	if (rem)
		rem->sign = F_SYMBOL(bn6400_bits)(rem) ? a_sign : 0;
	if (quo) {
		a_sign ^= b_sign;
		quo->sign = F_SYMBOL(bn6400_bits)(quo) ? a_sign : 0;
	}
}
		...
		const bn6400_t *a, uint32_t b)
{
	...
	F_SYMBOL(bn6400_udiv_1)(quo, rem, a, b);
		...
		const bn6400_t *a, const bn6400_t *b)
{
	...
	F_SYMBOL(bn6400_div)(quo, rem, a, b);

	if (rem && rem->sign) {
		F_SYMBOL(bn6400_add)(rem, rem, &t);
		rem->sign = 0;
	}
}

/* @func: bn6400_divmod_1
 * #desc:
...
void F_SYMBOL(bn6400_divmod_1)(bn6400_t *quo, bn6400_t *rem,
		const bn6400_t *a, uint32_t b)
{
	F_SYMBOL(bn6400_div_1)(quo, rem, a, b);

	if (rem && rem->sign) {
		F_SYMBOL(bn6400_add_1)(rem, rem, b);
		rem->sign = 0;
	}
}
		...
		const bn6400_t *a, const bn6400_t *e, const bn6400_t *m)
{
	...
	/* a1 = a % m */
	F_SYMBOL(bn6400_divmod)(NULL, &a1, a, m);
	...
	for (int32_t i = 0; i < n; i++) {
		...
		if (k_i & 1) {
			/* rr = (rr * a1) % m */
			F_SYMBOL(bn6400_mul)(&rr, &rr, &a1);
			F_SYMBOL(bn6400_divmod)(NULL, &rr, &rr, m);
		}

		/* a1 = (a1 * a1) % m */
		F_SYMBOL(bn6400_mul)(&a1, &a1, &a1);
		F_SYMBOL(bn6400_divmod)(NULL, &a1, &a1, m);
	}
		...
		const bn6400_t *a, const bn6400_t *b)
{
	...
	F_SYMBOL(bn6400_move)(&a1, a);
	F_SYMBOL(bn6400_move)(&b1, b);
	F_SYMBOL(bn6400_set1)(&s1, 1);
	F_SYMBOL(bn6400_set1)(&s2, 0);
	F_SYMBOL(bn6400_set1)(&t1, 0);
	F_SYMBOL(bn6400_set1)(&t2, 1);

	while (F_SYMBOL(bn6400_bits)(&b1)) {
		/* t3 = a1 % b1 */
		F_SYMBOL(bn6400_divmod)(&t3, NULL, &a1, &b1);

		/* (a1, b1) = (b1, a1 - (b1 * t3)) */
		F_SYMBOL(bn6400_mul)(&t4, &b1, &t3);
		F_SYMBOL(bn6400_sub)(&t4, &a1, &t4);
		F_SYMBOL(bn6400_move)(&a1, &b1);
		F_SYMBOL(bn6400_move)(&b1, &t4);

		/* (s1, s2) = (s2, s1 - (s2 * t3)) */
		F_SYMBOL(bn6400_mul)(&t4, &s2, &t3);
		F_SYMBOL(bn6400_sub)(&t4, &s1, &t4);
		F_SYMBOL(bn6400_move)(&s1, &s2);
		F_SYMBOL(bn6400_move)(&s2, &t4);

		/* (t1, t2) = (t2, t1 - (t2 * t3)) */
		F_SYMBOL(bn6400_mul)(&t4, &t2, &t3);
		F_SYMBOL(bn6400_sub)(&t4, &t1, &t4);
		F_SYMBOL(bn6400_move)(&t1, &t2);
		F_SYMBOL(bn6400_move)(&t2, &t4);
	}

	F_SYMBOL(bn6400_move)(r, &a1);
	F_SYMBOL(bn6400_move)(x, &s1);
	F_SYMBOL(bn6400_move)(y, &t1);
}
		...
		const bn6400_t *a, const bn6400_t *b)
{
	...
	F_SYMBOL(bn6400_gcd)(&g, &x, &y, a, b);
	if (F_SYMBOL(bn6400_cmp_1)(&g, 1)) {
		F_SYMBOL(bn6400_set1)(r, 0);
		return -1;
	}

	/* r = ((x % b) + b) % b */
	F_SYMBOL(bn6400_divmod)(NULL, &x, &x, b);
	F_SYMBOL(bn6400_add)(&x, &x, b);
	F_SYMBOL(bn6400_divmod)(NULL, r, &x, b);

	return 0;
}

/* @func: bn6400_redc_init
 * #desc:
...
{
	s->k = F_SYMBOL(bn6400_bits)(n);
	F_SYMBOL(bn6400_move)(&s->n, n);

	/* R = 1 << k */
	F_SYMBOL(bn6400_set1)(&s->R, 1);
	F_SYMBOL(bn6400_lshift)(&s->R, s->k);

	/* n_prime = -inv(n, R) */
	if (F_SYMBOL(bn6400_inv)(&s->np, n, &s->R))
		return -1;
	s->np.sign = 1;

	/* n_prime = n_prime % R */
	F_SYMBOL(bn6400_divmod)(NULL, &s->np, &s->np, &s->R);

	/* mask = R - 1 */
	F_SYMBOL(bn6400_sub_1)(&s->mask, &s->R, 1);

	/* R = (R * R) % n */
	F_SYMBOL(bn6400_mul)(&s->R, &s->R, &s->R);
	F_SYMBOL(bn6400_divmod)(NULL, &s->R, &s->R, n);

	return 0;
}

/* @func: bn6400_redc_mod
 * #desc:
		...
		bn6400_t *r, const bn6400_t *a)
{
	...
	F_SYMBOL(bn6400_and)(&t, a, &s->mask);
	F_SYMBOL(bn6400_mul)(&t, &t, &s->np);
	F_SYMBOL(bn6400_and)(&t, &t, &s->mask);

	/* t = (a + (t * n)) >> k */
	F_SYMBOL(bn6400_mul)(&t, &t, &s->n);
	F_SYMBOL(bn6400_add)(&t, &t, a);
	F_SYMBOL(bn6400_rshift)(&t, s->k);

	/* t = if t >= n : adjust */
	if (F_SYMBOL(bn6400_cmp)(&t, &s->n) >= 0)
		F_SYMBOL(bn6400_sub)(&t, &t, &s->n);
		...
		bn6400_t *r, const bn6400_t *a)
{
	...
	/* r = redc(a * R) */
	F_SYMBOL(bn6400_mul)(&t, a, &s->R);
	F_SYMBOL(bn6400_redc_mod)(s, r, &t);
}

/* @func: bn6400_redc_pow
 * #desc:
		...
		const bn6400_t *a, const bn6400_t *e, const bn6400_t *m)
{
	...
	if (F_SYMBOL(bn6400_redc_init)(&s, m))
		return -1;
	...
	for (int32_t i = 0; i < n; i++) {
		...
		if (k_i & 1) {
			/* rr = (rr * a1) % m */
			F_SYMBOL(bn6400_mul)(&rr, &rr, &a1);
			F_SYMBOL(bn6400_redc_mod)(&s, &rr, &rr);
		}

		/* a1 = (a1 * a1) % m */
		F_SYMBOL(bn6400_mul)(&a1, &a1, &a1);
		F_SYMBOL(bn6400_redc_mod)(&s, &a1, &a1);
	}

	/* r = rr % m */
	F_SYMBOL(bn6400_redc_mod)(&s, r, &rr);
		...
		const char *s, char **e, int32_t b)
{
	...
	for (int32_t d = 0; *s != '\0'; s++) {
		...
		F_SYMBOL(bn6400_umul_1)(t, t, b);
		F_SYMBOL(bn6400_uadd_1)(t, t, d);
	}
