...
{
	C_SYMBOL(memcpy)(t, s, sizeof(bn8448_t));
}
...
{
	bn8448_t t;
	C_SYMBOL(memcpy)(&t, a, sizeof(bn8448_t));
	C_SYMBOL(memcpy)(a, b, sizeof(bn8448_t));
	C_SYMBOL(memcpy)(b, &t, sizeof(bn8448_t));
}
...
{
	C_SYMBOL(memset)(t, 0, sizeof(bn8448_t));
	t->n[0] = n;
}

/* @func: bn8448_from_bytes
 * #desc:
...
{
	F_SYMBOL(bn8448_set1)(t, 0);
	C_SYMBOL(memcpy)(t->n, s, len);
}

/* @func: bn8448_to_bytes
 * #desc:
		...
		const bn8448_t *a, const bn8448_t *b)
{
	...
	F_SYMBOL(bn8448_set1)(&rr, 0);

	int32_t a_len = F_SYMBOL(bn8448_bits)(a);
	int32_t b_len = F_SYMBOL(bn8448_bits)(b);
	if (!a_len || !b_len)
		goto e;
...
{
	bn8448_t rquo, rrem;
	F_SYMBOL(bn8448_move)(&rrem, a);
	F_SYMBOL(bn8448_move)(&rquo, b);

	if (rem)
		F_SYMBOL(bn8448_move)(rem, &rrem);
	if (quo)
		F_SYMBOL(bn8448_set1)(quo, 0);

	if (!F_SYMBOL(bn8448_ucmp_1)(&rquo, 0)) {
		if (rem)
			F_SYMBOL(bn8448_set1)(rem, 0);
		return;
	}
	if (F_SYMBOL(bn8448_ucmp)(&rrem, &rquo) < 0)
		return;

	int32_t a_len = F_SYMBOL(bn8448_bits)(&rrem);
	int32_t b_len = F_SYMBOL(bn8448_bits)(&rquo);
	if (!a_len || !b_len)
		return;
...
{
	bn8448_t t;
	F_SYMBOL(bn8448_set1)(&t, b);

	F_SYMBOL(bn8448_udiv)(quo, rem, a, &t);
}
		...
		const bn8448_t *a, const bn8448_t *b)
{
	...
	if (sign == b->sign) {
		F_SYMBOL(bn8448_uadd)(r, a, b);
		r->sign = sign;
		return;
	}

	int32_t k = F_SYMBOL(bn8448_ucmp)(a, b);
	if (k > 0) {
		F_SYMBOL(bn8448_usub)(r, a, b);
		r->sign = sign;
	} else if (k < 0) {
		sign = b->sign;
		F_SYMBOL(bn8448_usub)(r, b, a);
		r->sign = sign;
	} else {
		F_SYMBOL(bn8448_set1)(r, 0);
	}
		...
		const bn8448_t *a, const bn8448_t *b)
{
	...
	F_SYMBOL(bn8448_umul)(r, a, b);

	r->sign = F_SYMBOL(bn8448_bits)(r) ? sign : 0;
}
		...
		const bn8448_t *a, const bn8448_t *b)
{
	...
	F_SYMBOL(bn8448_udiv)(quo, rem, a, b);

	if (rem)
		rem->sign = F_SYMBOL(bn8448_bits)(rem) ? a_sign : 0;
	if (quo) {
		a_sign ^= b_sign;
		quo->sign = F_SYMBOL(bn8448_bits)(quo) ? a_sign : 0;
	}
}
		...
		const bn8448_t *a, uint32_t b)
{
	...
	F_SYMBOL(bn8448_udiv_1)(quo, rem, a, b);
		...
		const bn8448_t *a, const bn8448_t *b)
{
	...
	F_SYMBOL(bn8448_div)(quo, rem, a, b);

	if (rem && rem->sign) {
		F_SYMBOL(bn8448_add)(rem, rem, &t);
		rem->sign = 0;
	}
}

/* @func: bn8448_divmod_1
 * #desc:
...
void F_SYMBOL(bn8448_divmod_1)(bn8448_t *quo, bn8448_t *rem,
		const bn8448_t *a, uint32_t b)
{
	F_SYMBOL(bn8448_div_1)(quo, rem, a, b);

	if (rem && rem->sign) {
		F_SYMBOL(bn8448_add_1)(rem, rem, b);
		rem->sign = 0;
	}
}
...
{
	bn8448_t rr, a1;
	F_SYMBOL(bn8448_set1)(&rr, 1);

	/* a1 = a % m */
	F_SYMBOL(bn8448_divmod)(NULL, &a1, a, m);

	int32_t n = F_SYMBOL(bn8448_bits)(e);
	for (int32_t i = 0; i < n; i++) {
		uint32_t k_i = e->n[i / 32] >> (i % 32);
		if (k_i & 1) {
			/* rr = (rr * a1) % m */
			F_SYMBOL(bn8448_mul)(&rr, &rr, &a1);
			F_SYMBOL(bn8448_divmod)(NULL, &rr, &rr, m);
		}

		/* a1 = (a1 * a1) % m */
		F_SYMBOL(bn8448_mul)(&a1, &a1, &a1);
		F_SYMBOL(bn8448_divmod)(NULL, &a1, &a1, m);
	}
		...
		const bn8448_t *a, const bn8448_t *b)
{
	...
	F_SYMBOL(bn8448_move)(&a1, a);
	F_SYMBOL(bn8448_move)(&b1, b);
	F_SYMBOL(bn8448_set1)(&s1, 1);
	F_SYMBOL(bn8448_set1)(&s2, 0);
	F_SYMBOL(bn8448_set1)(&t1, 0);
	F_SYMBOL(bn8448_set1)(&t2, 1);

	while (F_SYMBOL(bn8448_bits)(&b1)) {
		/* t3 = a1 % b1 */
		F_SYMBOL(bn8448_divmod)(&t3, NULL, &a1, &b1);

		/* (a1, b1) = (b1, a1 - (b1 * t3)) */
		F_SYMBOL(bn8448_mul)(&t4, &b1, &t3);
		F_SYMBOL(bn8448_sub)(&t4, &a1, &t4);
		F_SYMBOL(bn8448_move)(&a1, &b1);
		F_SYMBOL(bn8448_move)(&b1, &t4);

		/* (s1, s2) = (s2, s1 - (s2 * t3)) */
		F_SYMBOL(bn8448_mul)(&t4, &s2, &t3);
		F_SYMBOL(bn8448_sub)(&t4, &s1, &t4);
		F_SYMBOL(bn8448_move)(&s1, &s2);
		F_SYMBOL(bn8448_move)(&s2, &t4);

		/* (t1, t2) = (t2, t1 - (t2 * t3)) */
		F_SYMBOL(bn8448_mul)(&t4, &t2, &t3);
		F_SYMBOL(bn8448_sub)(&t4, &t1, &t4);
		F_SYMBOL(bn8448_move)(&t1, &t2);
		F_SYMBOL(bn8448_move)(&t2, &t4);
	}

	F_SYMBOL(bn8448_move)(r, &a1);
	F_SYMBOL(bn8448_move)(x, &s1);
	F_SYMBOL(bn8448_move)(y, &t1);
}
		...
		const bn8448_t *a, const bn8448_t *b)
{
	...
	F_SYMBOL(bn8448_gcd)(&g, &x, &y, a, b);
	if (F_SYMBOL(bn8448_cmp_1)(&g, 1)) {
		F_SYMBOL(bn8448_set1)(r, 0);
		return -1;
	}

	/* r = ((x % b) + b) % b */
	F_SYMBOL(bn8448_divmod)(NULL, &x, &x, b);
	F_SYMBOL(bn8448_add)(&x, &x, b);
	F_SYMBOL(bn8448_divmod)(NULL, r, &x, b);

	return 0;
}

/* @func: bn8448_redc_init
 * #desc:
...
{
	s->k = F_SYMBOL(bn8448_bits)(n);
	F_SYMBOL(bn8448_move)(&s->n, n);

	/* R = 1 << k */
	F_SYMBOL(bn8448_set1)(&s->R, 1);
	F_SYMBOL(bn8448_lshift)(&s->R, s->k);

	/* n_prime = -inv(n, R) */
	if (F_SYMBOL(bn8448_inv)(&s->np, n, &s->R))
		return -1;
	s->np.sign = 1;

	/* n_prime = n_prime % R */
	F_SYMBOL(bn8448_divmod)(NULL, &s->np, &s->np, &s->R);

	/* mask = R - 1 */
	F_SYMBOL(bn8448_sub_1)(&s->mask, &s->R, 1);

	/* R = (R * R) % n */
	F_SYMBOL(bn8448_mul)(&s->R, &s->R, &s->R);
	F_SYMBOL(bn8448_divmod)(NULL, &s->R, &s->R, n);

	return 0;
}

/* @func: bn8448_redc_mod
 * #desc:
		...
		bn8448_t *r, const bn8448_t *a)
{
	...
	F_SYMBOL(bn8448_and)(&t, a, &s->mask);
	F_SYMBOL(bn8448_mul)(&t, &t, &s->np);
	F_SYMBOL(bn8448_and)(&t, &t, &s->mask);

	/* t = (a + (t * n)) >> k */
	F_SYMBOL(bn8448_mul)(&t, &t, &s->n);
	F_SYMBOL(bn8448_add)(&t, &t, a);
	F_SYMBOL(bn8448_rshift)(&t, s->k);

	/* t = if t >= n : adjust */
	if (F_SYMBOL(bn8448_cmp)(&t, &s->n) >= 0)
		F_SYMBOL(bn8448_sub)(&t, &t, &s->n);

	F_SYMBOL(bn8448_move)(r, &t);
}

/* @func: bn8448_redc_mont
 * #desc:
		...
		bn8448_t *r, const bn8448_t *a)
{
	...
	/* r = redc(a * R) */
	F_SYMBOL(bn8448_mul)(&t, a, &s->R);
	F_SYMBOL(bn8448_redc_mod)(s, r, &t);
}

/* @func: bn8448_redc_pow
 * #desc:
		...
		const bn8448_t *a, const bn8448_t *e, const bn8448_t *m)
{
	...
	if (F_SYMBOL(bn8448_redc_init)(&s, m))
		return -1;

	F_SYMBOL(bn8448_set1)(&rr, 1);
	F_SYMBOL(bn8448_redc_mont)(&s, &rr, &rr);
	F_SYMBOL(bn8448_redc_mont)(&s, &a1, a);
	...
	for (int32_t i = 0; i < n; i++) {
		...
		if (k_i & 1) {
			/* rr = (rr * a1) % m */
			F_SYMBOL(bn8448_mul)(&rr, &rr, &a1);
			F_SYMBOL(bn8448_redc_mod)(&s, &rr, &rr);
		}

		/* a1 = (a1 * a1) % m */
		F_SYMBOL(bn8448_mul)(&a1, &a1, &a1);
		F_SYMBOL(bn8448_redc_mod)(&s, &a1, &a1);
	}

	/* r = rr % m */
	F_SYMBOL(bn8448_redc_mod)(&s, r, &rr);
		...
		const char *s, char **e, int32_t b)
{
	...
	for (int32_t d = 0; *s != '\0'; s++) {
		...
		F_SYMBOL(bn8448_umul_1)(t, t, b);
		F_SYMBOL(bn8448_uadd_1)(t, t, d);
	}
