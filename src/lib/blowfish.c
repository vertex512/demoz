		...
		uint32_t len)
{
	...
	for (int32_t i = 0; i < 18; i += 2) {
		F_SYMBOL(blowfish_encrypt)(ctx, &xl, &xr);
		ctx->P[i] = xl;
		ctx->P[i + 1] = xr;
	}
...
void F_SYMBOL(blowfish_init)(struct blowfish_ctx *ctx, const uint8_t *key,
		uint32_t len)
{
	C_SYMBOL(memcpy)(ctx->S1, orig_S1, sizeof(orig_S1));
	C_SYMBOL(memcpy)(ctx->S2, orig_S2, sizeof(orig_S2));
	C_SYMBOL(memcpy)(ctx->S3, orig_S3, sizeof(orig_S3));
	C_SYMBOL(memcpy)(ctx->S4, orig_S4, sizeof(orig_S4));

	/* len == 0 : skip set key */
	if (len)
		F_SYMBOL(blowfish_setkey)(ctx, key, len);
}
