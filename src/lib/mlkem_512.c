...
static void _poly_decompress_dv(const uint8_t *a, struct poly *r)
{
	...
	for (int32_t i = 0; i < MLKEM_N; i += 2) {
		t0 = ((uint16_t)(a[0] & 15) * MLKEM_Q + 8) >> 4;
		t1 = ((uint16_t)(a[0] >> 4) * MLKEM_Q + 8) >> 4;
		a++;
...
/* @func: mlkem512_pke_genkey
 * #desc:
 *    mlkem512 k-pke generate encryption and decryption key function.
 *
 * #1: nonce
 * #2: encryption key
 * #3: decryption key
		...
		uint8_t *dkp)
{
	...
	C_SYMBOL(memcpy)(ekp + MLKEM_512_POLYVEC_BYTES, seed, MLKEM_SYM_LEN);
	_polyvec_tobytes(&t, ekp);
	_polyvec_tobytes(&s, dkp);
}

/* @func: mlkem512_pke_encrypt
 * #desc:
 *    mlkem512 k-pke encryption function.
 *
 * #1: nonce
 * #2: encryption key
 * #3: input nmessage
		...
		const uint8_t *msg, uint8_t *ct)
{
	...
	C_SYMBOL(memcpy)(seed, ekp + MLKEM_512_POLYVEC_BYTES, MLKEM_SYM_LEN);
...
/* @func: mlkem512_pke_decrypt
 * #desc:
 *    mlkem512 k-pke decryption function.
 *
 * #1: decryption key
 * #2: input ciphertext
 * #3: output message
...
/* @func: mlkem512_genkey
 * #desc:
 *    mlkem512 generate encapsulation and decapsulation key function.
 *
 * #1: nonce of k-pke
 * #2: nonce of dk
 * #3: encapsulation key
		...
		uint8_t *ek, uint8_t *dk)
{
	...
	F_SYMBOL(mlkem512_pke_genkey)(ran, ek, dk);

	/* dk = dkp + ekp + hash_h(ekp) + ran2 */
	dk += MLKEM_512_DKPKE_LEN;
	C_SYMBOL(memcpy)(dk, ek, MLKEM_512_EK_LEN);
...
/* @func: mlkem512_encaps
 * #desc:
 *    mlkem512 encapsulation the function and generate a shared key.
 *
 * #1: nonce message
 * #2: encapsulation key
 * #3: output ciphertext
		...
		uint8_t *ct, uint8_t *sk)
{
	...
	F_SYMBOL(mlkem512_pke_encrypt)(r, ek, msg, ct);
}

/* @func: mlkem512_decaps
 * #desc:
 *    mlkem512 decapsulation the function and generate a shared key.
 *
 * #1: decapsulation key
 * #2: input ciphertext
 * #3: output shared key
		...
		uint8_t *sk)
{
	...
	F_SYMBOL(mlkem512_pke_decrypt)(dkp, ct, msg);
	...
	F_SYMBOL(mlkem512_pke_encrypt)(r, ekp, msg, ct2);
