		...
		uint8_t *dkp)
{
	...
	C_SYMBOL(memcpy)(ekp + MLKEM_1024_POLYVEC_BYTES, seed, MLKEM_SYM_LEN);
		...
		uint8_t *ek, uint8_t *dk)
{
	...
	F_SYMBOL(mlkem1024_pke_genkey)(ran, ek, dk);

	/* dk = dkp + ekp + hash_h(ekp) + ran2 */
	dk += MLKEM_1024_DKPKE_LEN;
	C_SYMBOL(memcpy)(dk, ek, MLKEM_1024_EK_LEN);
		...
		uint8_t *ct, uint8_t *sk)
{
	...
	F_SYMBOL(mlkem1024_pke_encrypt)(r, ek, msg, ct);
}
		...
		uint8_t *sk)
{
	...
	F_SYMBOL(mlkem1024_pke_decrypt)(dkp, ct, msg);
	...
	F_SYMBOL(mlkem1024_pke_encrypt)(r, ekp, msg, ct2);
